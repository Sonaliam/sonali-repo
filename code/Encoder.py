class Encoder(nn.Module):
    ...
    def forward(self, art, art_mask, imgs, ents, ients):
        ...
        ents_mask, ients_mask = None, None
        for layer in self.layers:
            art = layer(art, art_mask, imgs)
            ents = layer(ents, ents_mask, imgs)
            ients = layer(ients, ients_mask, imgs)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, art, art_mask, imgs):
        ...
        art_bar =  torch.empty(size=(1, 1, hid_dim)) #art1 + ((1-g) * art[:,0,:]) 
